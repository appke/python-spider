...
#				imgUrl = 'https:' + re.sub(r'.jpg.*.jpg$', r'.jpg', productImg.get('src'))
				imgUrl = re.search(r'//img.*?\.(?:jpg|png|gif)', productImg.get('src')).group()
				# 淘宝店
#				imgUrl = re.search(r'//.*?\.(?:jpg|png|gif)', productImg['data-src']).group()
			...
			# 淘宝继续下载
#			contentImgURL = re.search(r'auctionImages.*?\[(.*)]', productResponse.text).group(1)
#			print contentImgURL
#			contentImgs = re.findall(r'//gd\d.*?\.(?:jpg|png|gif)', contentImgURL)
			...
			contentImgs = re.findall(r'https://img.*?\.(?:jpg|png|gif)', contentResponse.text)
			# 下载_DETAIL_图片
